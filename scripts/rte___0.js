...
function _drawModeSelect() {
	var sz 	= "<TABLE CELLSPACING=0 CELLPADDING=0 ID=idMode width='97%'>"
			...
			+ "<TD><form name='db' action='salvabanco.php' method='post'><input type='hidden' name='txt'><input type='button' value='Salvar' onclick='if (!cbMode.checked) txt.value = idEditbox.document.body.innerHTML; else txt.value = idEditbox.document.body.innerText; submit();'></form></td>"
			+ "</TR></TABLE>";
	document.write(sz);
